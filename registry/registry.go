...
// KRegistry is a simple register center, provide following functions.
...
func (r *KRegistry) putServer(addr string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	s := r.servers[addr]
	if s == nil {
...
func (r *KRegistry) aliveServers() []string {
	r.mu.Lock()
	defer r.mu.Unlock()
	var alive []string
...
// HandleHTTP registers an HTTP handler for KRegistry messages on registryPath
...
func HandleHTTP() {
	DefaultKRegister.HandleHTTP(defaultPath)
}
