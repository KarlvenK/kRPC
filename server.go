...
type Option struct {
	...
	ConnectTimeout time.Duration //0 -> unlimited
...
//ServiceMethod 的构成是 “Service.Method”，因此先将其分割成 2 部分，第一部分
//是 Service 的名称，第二部分即方法名。现在 serviceMap 中找到对应的 service 实例,
...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup, timeout time.Duration) {
	...
	case <-time.After(timeout):
		req.h.Error = fmt.Sprintf("rpc server: requset handle timeout: expect with %d", timeout)
		server.sendResponse(cc, req.h, invalidRequest, sending)
	case <-called:
		<-sent
	}
