...
Done 的类型是 chan *Call，当调用结束时，会调用
call.done() 通知调用方。
*/
func (call *Call) done() {
	call.Done <- call
...
//There may be outstanding Calls associated
...
type Client struct {
	...
	header   codec.Header
	mu       sync.Mutex
	seq      uint64           //请求编号(用于给call编号，每次编号结束自增1）
	pending  map[uint64]*Call //存储未处理请求
	closing  bool             //用户主动关闭
	shutdown bool             //shutdown为true一般是有错误发生
}
...
//DialHTTP connects to an HTTP server at the specified network address
...
// eg, http@10.0.0.1:7001, tcp@10.0.0.1:9999, unix@/tmp/krpc.sock
